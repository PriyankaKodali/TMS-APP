...
class ThroughMe extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            TasksOnMe: [], active: false, SortModal: false, SortProps: sortProps, FilterModal: false,
    ...
    refreshTasks() {
        this.setState({ currentPage: 1, sizePerPage: 10, IsDataAvailable: false, TasksOnMe: [] }, () => {
            this.GetMyTasks(1, this.state.sizePerPage);
        })
    }
    ...
    GetMyTasks(page, count) {
        ...
        MyFetch(url, "GET", null).then((data) => {
            var tasksOnMe = this.state.TasksOnMe;
            tasksOnMe = tasksOnMe.concat(data["tasksThroughMe"]);
            this.setState({
                TasksOnMe: tasksOnMe, currentPage: page, sizePerPage: count, IsDataAvailable: true
            })
        }).catch((error) => {
    ...
    onEndReached = async () => {
        if (this.state.TasksOnMe.length !== this.state.currentPage * this.state.sizePerPage) {
            return;
        }
        this.setState({ IsDataAvailable: false });
        this.GetMyTasks(this.state.currentPage + 1, this.state.sizePerPage);
    }
        ...
        return (
            <View style={{ flexGrow: 1 }}>
                <List
                    ...
                    dataArray={this.state.TasksOnMe}
                    renderRow={data => {
                        return (
                            <ListItem button onPress={() => this.props.navigation.navigate('TaskDetail', { TaskId: data.TaskId })}
                                style={[{ marginLeft: 0 }, data["Status"] === "Open" || data["Status"] === "Reopened" ? MasterStyles.open : data["Status"] === "Pending" ? MasterStyles.pending : MasterStyles.closed]}>
                                <Body >
                ...
                <Modal
                    ...
                    <ScrollView style={{ flex: 1, flexDirection: "column" }}>
                        ...
                        <Button full info onPress={() => { this.setState({ FilterModal: false, IsDataAvailable: false, TasksOnMe: [] }); this.GetMyTasks(1, this.state.sizePerPage) }}>
                            <Text>Ok</Text>
                        </Button>
                    </ScrollView>
                </Modal>
    ...
    sortDesc(column) {
        this.setState({ TasksOnMe: [], sortDir: "desc", sortCol: column, IsDataAvailable: false, currentPage: 1, SortModal: false }, () => {
            this.GetMyTasks(this.state.currentPage, this.state.sizePerPage);
        })
    }

    sortAsc(column) {
        this.setState({ TasksOnMe: [], sortDir: "asc", sortCol: column, IsDataAvailable: false, currentPage: 1, SortModal: false }, () => {
            this.GetMyTasks(this.state.currentPage, this.state.sizePerPage);
        })
    }
