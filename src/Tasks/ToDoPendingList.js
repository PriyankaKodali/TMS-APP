...
import {
    ...
    Right, Spinner, Text, Title, Toast, Badge
} from "native-base";
...
class ToDoPendingList extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            PendingTasksOnMe: [], active: false, SortModal: false, SortProps: sortProps, FilterModal: false,
    ...
    refreshTasks() {
        this.setState({ currentPage: 1, sizePerPage: 10, IsDataAvailable: false, PendingTasksOnMe: [] }, () => {
    ...
    GetMyTasks(page, count) {
        ...
        MyFetch(url, "GET", null).then((data) => {
            var pendingTasksOnMe = this.state.PendingTasksOnMe;
            pendingTasksOnMe = pendingTasksOnMe.concat(data["myPendingTasks"]);
            this.setState({
                PendingTasksOnMe: pendingTasksOnMe, currentPage: page, sizePerPage: count, IsDataAvailable: true
            })
        }).catch((error) => {
    ...
    onEndReached = async () => {
        if (this.state.PendingTasksOnMe.length !== this.state.currentPage * this.state.sizePerPage) {
            return;
        }
        ...
        return (
            <View style={{ flexGrow: 1 }}>
                <List
                    ...
                    dataArray={this.state.PendingTasksOnMe}
                    renderRow={data => {
                        return (
    ...
    sortDesc(column) {
        this.setState({ PendingTasksOnMe: [], sortDir: "desc", sortCol: column, IsDataAvailable: false, currentPage: 1, SortModal: false }, () => {
            this.GetMyTasks(this.state.currentPage, this.state.sizePerPage);
        })
    }

    sortAsc(column) {
        this.setState({ PendingTasksOnMe: [], sortDir: "asc", sortCol: column, IsDataAvailable: false, currentPage: 1, SortModal: false }, () => {
            this.GetMyTasks(this.state.currentPage, this.state.sizePerPage);
        })
    }
